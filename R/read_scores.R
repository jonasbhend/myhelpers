...
#' @param indices character vector of variable names
...
#' @keywords util
#' @export
read_scores <- function(scores=c('EnsCorr', 'FairRpss'), indices='tas',
  ...
  for (grid in grids){
    ...
    skill <- expand.grid(index=indices, method=methods, initmon=initmonths,
    ...
    filenames <- paste0('^', skill$index, c('', '_detrend')[skill$detrend*1 + 1],
                        ...
                        skill$method, '_.*initmon', skill$initmon, '.nc$')
