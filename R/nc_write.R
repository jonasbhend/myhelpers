...
nc_write <- function(nctempfile, file, varname, data, append=FALSE, ...){
  ...
  nctemplate <- ncdf4::nc_open(nctempfile, readunlim=FALSE, suppress_dimvals=TRUE)
  on.exit(ncdf4::nc_close(nctemplate))
  ...
  if (append & file.exists(file)){
    ncout <- ncdf4::nc_open(filename=file, write=TRUE, readunlim=FALSE, suppress_dimvals=TRUE)
  } else {
  ...
  ## write the data
  ncdf4::ncvar_put(ncout, varid=varname, vals=data, ...)
